...
class CoreDataHelper: NSObject {
    ...
    static var context:NSManagedObjectContext{
        return CoreDataHelper.helper.persistentContainer.viewContext
    }
    ...
    func deleteManagedObject(index:Int){
        do {
            let context = CoreDataHelper.context
            let fetchRequest:NSFetchRequest<Swords> = Swords.fetchRequest()
            let mo = try context.fetch(fetchRequest)
            context.delete(mo[index])
            do {
                try context.save()
            } catch let errorSave {
                print(errorSave)
